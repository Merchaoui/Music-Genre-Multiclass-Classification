...
function modelCrossValidation(modelType::Symbol,
    ...
    if (modelType == :SVM)
        ...
        model = SVC(kernel=modelHyperParameters["kernel"], degree=modelHyperParameters["kernelDegree"], 
            gamma = modelHyperParameters["kernelGamma"], C=modelHyperParameters["C"], tol=tol, coef0=coef0, 
    ...
    elseif (modelType == :LR)    
        ...
        C = get(modelHyperParameters,"C",1)
        solver = get(modelHyperParameters,"solver","lbfgs")

        model = LogisticRegression(max_iter=max_iter, multi_class=multi_class, C=C, solver=solver)
    elseif (modelType == :NC)    
    ...
    elseif (modelType == :RF)    
        n_estimators = get(modelHyperParameters,"n_estimators",180)
        max_depth = get(modelHyperParameters,"max_depth",11)
        max_features = get(modelHyperParameters,"max_features","auto")
